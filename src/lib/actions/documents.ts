...
export type DocType = "quotation" | "invoice" | "proforma";
...
/** Builds e.g. QTN-2026-KGIS010 or PI-2026-KGIS010 from the numbering config. */
...
export async function saveDocument(
  ...
  const base = {
    ...
    exportScheme: v.exportScheme ?? "lut",
    exportLutNumber: v.exportLutNumber ?? null,
    exportLutDate: v.exportLutDate ?? null,
    portOfLoading: v.portOfLoading ?? null,
    portOfDischarge: v.portOfDischarge ?? null,
    incoterms: v.incoterms ?? null,
    modeOfShipment: v.modeOfShipment ?? null,
    countryOfOrigin: v.countryOfOrigin ?? "India",
    totalPackages: v.totalPackages ?? null,
...
/**
 * Copies a quotation into a new proforma invoice.
 */
export async function convertToProforma(
  ...
  if (source.type !== "quotation") {
    return { ok: false, error: "Only quotations can be converted to proforma invoices." };
  }
  ...
  try {
    const { id: proformaId, number } = await insertWithNextNumber("proforma", {
      type: "proforma",
      status: "draft",
      ...
      exportScheme: source.exportScheme,
      exportLutNumber: source.exportLutNumber,
      exportLutDate: source.exportLutDate,
      portOfLoading: source.portOfLoading,
      portOfDischarge: source.portOfDischarge,
      incoterms: source.incoterms,
      modeOfShipment: source.modeOfShipment,
      countryOfOrigin: source.countryOfOrigin,
      totalPackages: source.totalPackages,
    ...
    await db
      .update(documents)
      .set({ convertedToId: proformaId, status: "accepted" })
      .where(eq(documents.id, quotationId));

    revalidatePath("/quotations");
    revalidatePath("/proformas");
    revalidatePath("/invoices");
    return { ok: true, data: { id: proformaId, number } };
  } catch (e) {
...
/**
 * Copies a quotation or proforma invoice into a new tax invoice.
 */
export async function convertToInvoice(
  ...
  if (source.type === "invoice") {
    return { ok: false, error: "Invoices cannot be converted to another invoice." };
  }
  if (source.convertedToId) {
    return {
      ok: false,
      error: "This document has already been converted to an invoice.",
    };
  }
...
export async function dashboardStats(): Promise<{
  ...
  return {
    ...
    proformaCount: rows.filter((r) => r.type === "proforma").length,
